...
public class IspybDataCollectionDAO implements IspybDataCollectionApi {
	...
	private Object[] getParameters(DataCollectionGroup dataCollectionGroup) {
		return getParametersInOrder(dataCollectionGroup, "id", "proposalCode", "proposalNumber", "sessionNumber",
				"sampleId", "sampleBarcode", "experimenttype", "starttime", "endtime", "crystalClass", "detectorMode",
				"actualSampleBarcode", "actualSampleSlotInContainer", "actualContainerBarcode",
				"actualContainerSlotInSC", "comments");
	}

	private Object[] getParameters(DataCollectionMain dataCollectionMain) {
		return getParametersInOrder(dataCollectionMain, "id", "groupId", "detectorId", "dcNumber", "startTime",
				"endTime", "status", "noImages", "startImgNumber", "noPasses", "imgDir", "imgPrefix", "imgSuffix",
				"fileTemplate", "snapshot1", "snapshot2", "snapshot3", "snapshot4", "comments");
	}

	private Object[] getParameters(DataCollectionExperiment dataCollectionExperiment) {
		return getParametersInOrder(dataCollectionExperiment, "id", "slitGapVertical", "slitGapHorizontal",
				"transmission", "exposureTime", "xBeam", "yBeam", "axisStart", "axisEnd", "axisRange", "overlap",
				"flux", "fluxEnd", "rotationAxis", "phiStart", "kappaStart", "omegaStart", "wavelength", "resolution",
				"detectorDistance", "bestWilsonPlotPath", "beamSizeAtSampleX", "beamSizeAtSampleY",
				"focalSpotSizeAtSampleX", "focalSpotSizeAtSampleY", "apertureSizeX");
	}

	private Object[] getParameters(DataCollectionMachine dataCollectionMachine) {
		return getParametersInOrder(dataCollectionMachine, "id", "synchrotronMode", "undulatorGap1", "undulatorGap2",
				"undulatorGap3");
	}

	private Object[] getParametersInOrder(Object bean, String... order) {
		BeanWrapper wrapper = PropertyAccessorFactory.forBeanPropertyAccess(bean);
		return Stream.of(order).map(property -> {
			try {
