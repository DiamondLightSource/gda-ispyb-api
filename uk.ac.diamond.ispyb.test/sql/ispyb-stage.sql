...
) ENGINE=InnoDB AUTO_INCREMENT=34878 DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `ContainerQueue` VALUES (2,34874,NULL,'2016-09-30 12:56:21','2016-10-18 14:47:56'),(8,34877,NULL,'2016-10-05 09:09:59','2016-10-18 14:47:56');
...
WHERE c.beamlineLocation = p_beamline AND c.containerStatus = 'In imager' AND cq.completedTimeStamp is NULL
...
BEGIN
    IF NOT (p_barcode IS NULL) THEN 
      ...
      FROM Container 
      WHERE barcode = p_barcode AND containerStatus = 'In local storage'
...
BEGIN
  IF NOT (p_beamline IS NULL) THEN
    ...
    FROM Container c
      INNER JOIN ContainerQueue cq ON c.containerId = cq.containerId
    WHERE cq.completedTimeStamp is NULL AND c.beamlineLocation = p_beamline AND c.containerStatus = 'In local storage'
	ORDER BY cq.createdTimeStamp ASC;
  END IF;
END ;;
DELIMITER ;
...
BEGIN
  IF NOT (p_barcode IS NULL) THEN
    ...
    FROM Container c 
      ...
      INNER JOIN DiffractionPlan dp ON dp.diffractionPlanId = blss.diffractionPlanId
      LEFT OUTER JOIN BLSampleImage blsi ON blsi.blSampleId = bls.blSampleId AND blsi.blSampleImageId = (SELECT blsi2.blSampleImageId FROM BLSampleImage blsi2 WHERE blsi2.blSampleId = bls.blSampleId)
...
CREATE PROCEDURE `update_container_status`(IN p_barcode varchar(45), IN p_status varchar(45))
    MODIFIES SQL DATA
BEGIN
  IF NOT (p_barcode IS NULL) AND p_status IN ('In local storage', 'In transit', 'In imager') THEN
    UPDATE Container
...
-- Dump completed on 2016-11-03 12:15:09
